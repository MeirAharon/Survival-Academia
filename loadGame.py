...
def creatLevel(app):
    ...
    for i, row in enumerate(levelFile.readlines()):
        app.levelRows += 1
        for j, col in enumerate(row.split(' ')):
            if col != '-1' and col != '\n' and col != '':
                x = int(j * Tile.tileWidth)
                y = int(i * Tile.tileHeight)
                app.tileList.append(Tile(x, y, int(col)))
                app.levelCols += 1
                print(app.height, 'x:', x, 'y:', y, j, i, Tile.tileHeight)
    
def createCollisionBoard(app):
    
    app.tileDict = dict()
    for row in range(app.levelRows):
        for col in range(app.levelCols):
            app.tileDict[(row, col)] = []       
    for tile in app.tileList:
        row = int(tile.y // Tile.tileHeight)
        col = int(tile.x // Tile.tileWidth)
        print(tile.y, tile.x)
...
def drawLevel(app):
    ...
    for tile in app.tileList:
        drawImage(tile.img, tile.x, tile.y, width=Tile.tileWidth, height=Tile.tileHeight, fill = 'yellow')
...
class Tile():
    ...
    print(app.height)
    tileWidth = 60
    tileHeight = app.height // 12

    def __init__(self, x, y, imgNum):
        
        self.x = x
        self.y = y
        self.img = openImage(f"Tiles\\Tile_{imgNum}.png")
